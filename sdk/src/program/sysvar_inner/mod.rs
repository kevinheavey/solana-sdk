...
macro_rules! impl_sysvar_get {
    ($syscall_name:ident) => {
        fn get() -> Result<Self, $crate::program::sysvar_inner::__private::ProgramError> {
            let mut var = Self::default();
            let var_addr = &mut var as *mut _ as *mut u8;

            #[cfg(target_os = "solana")]
            let result = unsafe {
                $crate::program::sysvar_inner::__private::definitions::$syscall_name(var_addr)
            };

            #[cfg(not(target_os = "solana"))]
            let result = $crate::program::sysvar_inner::program_stubs::$syscall_name(var_addr);

            match result {
                $crate::program::sysvar_inner::__private::SUCCESS => Ok(var),
                // Unexpected errors are folded into `UnsupportedSysvar`.
                _ => Err($crate::program::sysvar_inner::__private::ProgramError::UnsupportedSysvar),
            }
        }
    };
}
